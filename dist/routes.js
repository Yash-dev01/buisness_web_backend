...
apiRouter.put('/settings', requireAuth, async (req, res) => {
    try {
        ...
        const settings = await StoreSettingsModel.findOneAndUpdate({}, { $set: updateData }, {
            new: true,
            upsert: true,
            runValidators: true,
...
apiRouter.put('/categories/:id', requireAuth, async (req, res) => {
    try {
        if (!requireDatabase(res))
            return;
        const { id } = req.params;
...
apiRouter.post('/products', requireAuth, async (req, res) => {
    try {
        if (!requireDatabase(res))
            return;
        const { name, price, categoryId, description, images, available, featured, } = req.body;
...
// BULK PRICE PREVIEW
// ------------------------------------------------------------
apiRouter.post('/products/bulk-preview', requireAuth, async (req, res) => {
    try {
        if (!requireDatabase(res))
            return;
...
// BULK OPERATIONS
// ------------------------------------------------------------
apiRouter.post('/products/bulk', requireAuth, async (req, res) => {
    try {
        if (!requireDatabase(res))
            return;
        const { action, productIds, adjustment, categoryId, available, } = req.body;
        if (!Array.isArray(productIds) ||
            productIds.length === 0) {
            res.status(400).json({
        ...
        if (action === 'price') {
            ...
            const bulkOperations = products.map((product) => ({
                updateOne: {
                    ...
                    update: {
                        $set: {
                            price: calculateNewPrice(product.price, adjustment),
                        },
                    },
                },
            }));
...
// IMAGE UPLOAD
// ------------------------------------------------------------
apiRouter.post('/upload', requireAuth, async (req, res) => {
    try {
...
apiRouter.get('/stats', requireAuth, async (req, res) => {
    try {
        if (!requireDatabase(res))
            return;
        const [totalProducts, inStockProducts, outOfStockProducts, whatsappOrders,] = await Promise.all([
...
apiRouter.post('/orders/log', async (req, res) => {
    try {
        if (!requireDatabase(res))
            return;
        const { productId, productName, price, categoryName, whatsappUrl, } = req.body;
        await OrderActivityModel.create({
            productId: productId || 'unknown',
            productName: productName || 'Catalogue Product',
